...
class MUDConnection:
    ...
    def login(self, name: str = DEFAULT_NAME, password: str = DEFAULT_PASSWORD) -> bool:
        ...
        resp = self.send_and_read(name, ["Password:", "already a name", "new character", "(Y/N)"], timeout=30.0)
        if "(Y/N)" in resp and "Password:" not in resp:
            resp = self.send_and_read("Y", ["Password:", "already a name", "new character"], timeout=30.0)
